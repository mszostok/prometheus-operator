...
export REPO="${REPO:-"quay.io/mszostok/prometheus-operator-dev"}"
export REPO_PROMETHEUS_CONFIG_RELOADER="${REPO_PROMETHEUS_CONFIG_RELOADER:-"quay.io/mszostok/prometheus-config-reloader-dev"}"

# Use main image repositories if TAG is a semver tag or it is a master branch
if [[ "$TAG" =~ ^v[0-9]+\.[0-9]+ ]] || [ "${TAG}" == "master" ]; then
	export REPO="quay.io/mszostok/prometheus-operator"
	export REPO_PROMETHEUS_CONFIG_RELOADER="quay.io/mszostok/prometheus-config-reloader"
fi
